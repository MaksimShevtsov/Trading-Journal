...
def test_select_field_requires_choices() -> None:
    with pytest.raises(ValueError, match="choices"):
        ResourceAdmin(
            ...
            dao=_dao(),
            list_columns=[ColumnConfig(name="id", label="ID")],
            form_fields=[FieldConfig(name="status", label="Status", field_type=FieldType.SELECT)],
        )
...
def test_all_field_types() -> None:
    r = ResourceAdmin(
        ...
        form_fields=[
            ...
            FieldConfig(name="timestamp", label="Timestamp", field_type=FieldType.DATETIME),
            FieldConfig(
                name="status",
                label="Status",
            ...
            FieldConfig(name="desc", label="Description", field_type=FieldType.TEXTAREA),
        ],
    )
