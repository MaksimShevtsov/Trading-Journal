...
class InMemoryUserDAO(AdminDAO):
    ...
    async def list(self, offset: int, limit: int, search: str | None = None) -> tuple[list[dict[str, Any]], int]:
        items = list(self._store.values())
        if search:
            term = search.lower()
            items = [item for item in items if term in item["name"].lower() or term in item["email"].lower()]
        total = len(items)
        if limit > 0:
