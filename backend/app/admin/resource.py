...
class ResourceAdmin:
    ...
    def __post_init__(self) -> None:
        if not _SLUG_RE.match(self.name):
            raise ValueError(f"Resource name '{self.name}' must be lowercase alphanumeric with hyphens")
        if not self.list_columns:
            raise ValueError("list_columns must not be empty")
        if not self.form_fields:
            raise ValueError("form_fields must not be empty")

        all_field_names = {c.name for c in self.list_columns} | {f.name for f in self.form_fields}
        if self.id_field not in all_field_names:
            raise ValueError(f"id_field '{self.id_field}' must exist in list_columns or form_fields")
