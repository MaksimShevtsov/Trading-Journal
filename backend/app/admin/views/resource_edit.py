...
def build_edit_routes(router: APIRouter, site: AdminSite) -> None:
    ...
    async def resource_edit_form(request: Request, resource_name: str, record_id: str) -> Response:
    ...
    async def resource_edit_submit(request: Request, resource_name: str, record_id: str) -> Response:
        user = await require_admin(request)
        if isinstance(user, RedirectResponse):
            return user
        ...
        if not validate_csrf(request, raw_data.get("csrf_token")):
            ...
            return RedirectResponse(url=f"{site.prefix}/{resource_name}/{record_id}/edit", status_code=303)
