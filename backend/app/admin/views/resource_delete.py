...
def build_delete_routes(router: APIRouter, site: AdminSite) -> None:
    ...
    async def resource_delete_confirm(request: Request, resource_name: str, record_id: str) -> Response:
    ...
    async def resource_delete_submit(request: Request, resource_name: str, record_id: str) -> Response:
        user = await require_admin(request)
        if isinstance(user, RedirectResponse):
            return user
        ...
        if not validate_csrf(request, submitted_token):
            ...
            return RedirectResponse(url=f"{site.prefix}/{resource_name}/{record_id}/delete", status_code=303)

        try:
            await resource.dao.delete(record_id)
            set_flash(request, "success", "Record deleted.")
            return RedirectResponse(url=f"{site.prefix}/{resource_name}/", status_code=303)
        except Exception:
            logger.exception("Error deleting record '%s' for resource '%s'", record_id, resource_name)
