...
def build_create_routes(router: APIRouter, site: AdminSite) -> None:
    ...
    @router.get("/{resource_name}/create", response_class=HTMLResponse, response_model=None)
    ...
    @router.post("/{resource_name}/create", response_class=HTMLResponse, response_model=None)
    async def resource_create_submit(request: Request, resource_name: str) -> Response:
        ...
        if not validate_csrf(request, raw_data.get("csrf_token")):
            ...
            return RedirectResponse(url=f"{site.prefix}/{resource_name}/create", status_code=303)
